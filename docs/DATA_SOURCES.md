...
| **Derivation Research Console** | Multi-tool LLM research briefs (idea-chats). Tools (Yahoo, Exa, SEC, options research, etc.) run server-side. Factory / Experiments / Jobs UI stays operator-only — not exposed in Mapvest. Prefer Railway host over workers.dev (Cloudflare Access). | `DERIVATION_URL` (default `https://derivation-research-console-production.up.railway.app`). Mapvest: `POST /v1/agent/chat`, `GET /v1/agent/threads`, `GET /v1/agent/threads/:id`. Product IA: Research tab (thread list + article composer) on web + iOS; ticker-bound Research… panel; Saved → Briefs archive. Broker orders permanently off. | jawauntb (sibling service) |
