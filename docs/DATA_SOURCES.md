...
| **Situate** (reforms Prism) — hosted inside [`The Underlying Analyzer Reboot`](https://github.com/jawauntb/the-underlying-analyzer-reboot) | The single-name research engine that *situates* a stock rather than forecasting its price: factor + macro **exposure** (EWMA-ridge betas, 12-month change), vol×trend **state** (+ optional HMM second opinion, VIX/HY/curve context), empirical **base rates** per horizon (conditional shrunk toward unconditional, with `n_eff`/`shrink_w`), the options-**implied** distribution (risk-neutral density from the smile, `width_ratio` vs history), **fundamentals** trajectory (momentum/quality/value-z; revisions & PEAD are `null` — Massive has no estimates endpoint), filing-diff **text** with quoted evidence + dated events, cheap/rich **zones** on the price ladder, a validated cross-sectional **stack** (or a stated fallback), merged **odds** per horizon, and an authored **memo** whose call is a *posture* (odds favorable / balanced / odds unfavorable — never buy/sell) with three falsifiers. Walk-forward with no data after `as_of`; every quantitative claim cites its module + version; unreachable sources are listed in `meta.errors`/`meta.unavailable`, never zero-filled. | `POST /v1/situate` (metered as `memo`, 180s budget), `GET /v1/situate/{ticker}`, `GET /v1/situate/{ticker}/summary`, `POST /v1/situate/chat`, `GET /v1/situate/{ticker}/export?format=txt\|json\|pdf`. Same handlers under the `/v1/research` alias. `POST /v1/agent/chat` pre-loads the summary as research context (preferred over Prism), best-effort with a 3s budget. Origin: `UNDERLYING_URL`. See `docs/SITUATE.md`. | jawauntb (sibling repo, separate service) |
