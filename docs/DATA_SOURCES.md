...
| [`The Underlying Analyzer Reboot`](https://github.com/jawauntb/the-underlying-analyzer-reboot) | Auction charts, Anthropic stock briefs / memos, SEC packs. Live at `underlying-terminal-production.up.railway.app`. | `GET /v1/chart/auction?ticker=MCD&period=1m` (PNG base64 + POC/VAH/VAL), `POST /v1/memo {ticker}` → analysis brief, `GET /v1/memo/sec/:ticker`. Detail screens auto-load the 1m auction chart for the ticker (or top comparable). | jawauntb (sibling repo, separate service) |
