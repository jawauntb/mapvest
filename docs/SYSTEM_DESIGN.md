...
- Checkout is channel-aware: `ios` + `APPLE_IAP_PRODUCT_ID` → `{ channel: "apple_iap", productId }` (App Store 3.1.1); `android` + `GOOGLE_PLAY_PRODUCT_ID` → Play Billing (deferred; do not ship a Play build from this); otherwise Stripe Checkout URL. Native IAP/Play purchase UI is a follow-up once those product ids exist. Until then TestFlight opens Stripe in Safari.
