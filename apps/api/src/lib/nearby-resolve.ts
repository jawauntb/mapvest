 ...
 * (full map/list payload) and `/v1/widget/*` (trimmed home-screen widget
 * payload). Extracted from the original `/v1/nearby` handler so the two
 * routes never drift on the places cascade or brand→ticker join.
 */
...
export type ResolveNearbyResult = {
  items: NearbyResponse["items"];
  placesSource: string;
};

/**
 * Cascade: Google Places (multi-type) → Overpass → Photon. Joins with the
 * brand→ticker resolver (+ validated comparables for private brands).
 *
 * If MOCK_PLACES is set (and NODE_ENV !== "production", unless
 * MOCK_PLACES_ALLOW_PROD=1) we read the fixture from that path instead of
 * calling Google — used by tests and the deployed demo before
 * GOOGLE_MAPS_API_KEY is provisioned.
 */
      ...
      if (!googleOk) {
        try {
          ...
          try {
            ...
            throw new Error(
              `places lookup failed: ${(overpassErr as Error).message}; photon: ${(photonErr as Error).message}`,
            );
          }
        }
      }
