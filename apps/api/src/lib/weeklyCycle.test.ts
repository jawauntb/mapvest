...
import { cycleWindow, isCycleCloseTick, msUntilClose } from "./weeklyCycle.js";
...
describe("cycleWindow", () => {
  ...
  test("Sunday still counts down to the same-week Saturday close", () => {
    const now = new Date("2026-09-13T00:00:00.000Z"); // Sunday, right after last close
...
describe("isCycleCloseTick", () => {
  test("true only at Saturday 12:00 UTC, on the minute", () => {
    ...
    expect(isCycleCloseTick(new Date("2026-09-19T12:00:59.000Z"))).toBe(true);
    expect(isCycleCloseTick(new Date("2026-09-19T11:59:00.000Z"))).toBe(false);
    expect(isCycleCloseTick(new Date("2026-09-19T13:00:00.000Z"))).toBe(false);
