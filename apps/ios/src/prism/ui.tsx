...
/**
 * The card every Prism section lives in.
 *
...
/** Label / value row for dense readouts (ratios, macro detail, filings meta). */
 ...
 * answer's footer).
 *
 * Deliberately NOT a `KeyValueRow`: that row clips its label to one line and
 * lets the value take its natural width, which on real packets truncates the
 * claim ("NVDA September mean return over the last 10 years: -0.76%") while
 * rendering the machine-readable path ("prism.seasonality.ticker.this_month.10y")
 * in full and in bold. The claim is the half a person can read, so it wraps and
 * comes first; the source is the caption under it.
 */
    ...
    <View style={styles.segment}>
      {options.map((option) => {
        ...
        return (
          <Pressable
            key={option}
            // Every other chip row in the app (ChartsSection, BacktestCard)
            // ticks on selection; these switch a whole chart and should feel
            // the same. Silent on a re-tap of the active chip.
            onPress={() => {
              if (active) return;
              hapticSelect();
              onChange(option);
 ...
 * mounted stays mounted — remounting a chart mid-scroll is worse than the
 * memory it saves.
 */
export const ScrollReachContext = createContext<number>(Number.POSITIVE_INFINITY);

/**
 * Defers a heavy section until the reader is within a viewport of it. Until
 * then it reserves `minHeight` so the scrollbar does not jump when it lands.
 */
...
const styles = StyleSheet.create({
  ...
  unavailable: {
    // The reason a section is empty is content, not chrome: fgMuted, not fgDim.
    color: colors.fgMuted,
    fontSize: 12.5,
    lineHeight: 18,
    fontStyle: "italic",
