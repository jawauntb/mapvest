...
/**
 * Self-measuring plot surface for the Situate charts — the Atlas Signal sibling
 * of `chartkit/primitives.tsx`'s `Panel`. Children render only once the width
 * is known, which keeps the View-backed SVG shims from laying out against a
 * zero-width parent.
 */
export function SituatePanel({
  height,
  style,
  children,
}: {
...
/** What a chart renders instead of an empty plot — an absent series says so in words. */
export function SituateChartEmpty({ note }: { note: string }) {
  return (
