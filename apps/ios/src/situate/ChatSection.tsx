import type { SituateCitation } from "@/api/situate";
...
import { type SituateChatTurn, useSituateChat } from "./useChat";

const SUGGESTIONS = [
  "What would prove this posture wrong?",
  "How do the options disagree with history?",
  "What is doing the most work in the odds?",
];

/**
 * Ask the packet a question. The engine answers strictly from the stored
 * packet, so this is not a general chat — it is an interrogation of numbers
 * already on this screen, with citations back into the modules that produced
 * them.
 */
export function SituateChatSection({ ticker, token }: { ticker: string; token?: string }) {
  const chat = useSituateChat(ticker, token);
    ...
    >
      ...
      <View style={styles.composer}>
        <TextInput
          ...
          placeholderTextColor={SITUATE_DIM}
          style={styles.input}
          multiline
          maxLength={4000}
...
function TurnCitations({ turn }: { turn: SituateChatTurn }) {
  const citations: SituateCitation[] = turn.citations ?? [];
...
const styles = StyleSheet.create({
  ...
  turn: { borderRadius: radii.md, padding: space.md, gap: 4, maxWidth: "94%" },
  ...
  citationToggle: { color: colors.accent, fontSize: 11.5, fontWeight: "700", paddingVertical: 4 },
